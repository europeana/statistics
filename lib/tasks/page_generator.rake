namespace :page_generator do 
  ...
  task :ga_traffic, [:name, :id]  do |t, args|
    ...
    tmp_data = ga.get({ :start_date => '2013-01-01', :end_date => '2013-12-31', :dimensions => ['month', 'year', 'hostname', 'pagePath'], :metrics => ['pageviews'],:filters => ['hostname == www.europeana.eu', "pagePath =~ /record/#{provider_id}"], max_results: 999999999})
    ...
    tmp_data = ga.get({ :start_date => '2013-01-01', :end_date => '2013-12-31', :dimensions => ['month', 'year', 'hostname', 'pagePath', 'eventCategory'], :metrics => ['totalEvents'],:filters => ['hostname==www.europeana.eu', "pagePath=~/record/#{provider_id}", 'eventCategory=~ Redirect'], max_results: 999999999}) 
    tmp_data.points.each do |d|
      custom_regex = "#{provider_id}"
      d.dimensions.each do |c|          
        c.each do |k,v|
      ...
      if !page_event_aggr[custom_regex]
        page_event_aggr[custom_regex] = d.metrics[0][:totalEvents].to_i
      else  
        page_event_aggr[custom_regex] = page_event_aggr[custom_regex] + d.metrics[0][:totalEvents].to_i
      end
    end
  ...
  task :article, :params  do |t, args|
    ...
    if article.nil?
      ...
      html_template += "<h1>Ab de Villiars</h1> </div>"
