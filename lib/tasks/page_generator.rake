  ...
  task :ga_traffic, [:name, :id, :provider_type,:wiki_name]  do |t, args|
    ...
    provider_ids.each do |p|
      comma = "," 
      comma = ""  if provider_ids[-1] == p 
      ga_filters += "ga:pagePath=~/#{p}/#{comma}"
    end
    ...
    ga_filters2    = "#{ga_filters};ga:eventCategory=~Redirect"

    tmp_data = JSON.parse(open("https://www.googleapis.com/analytics/v3/data/ga?access_token=#{access_token}&start-date=#{ga_start_date}&end-date=#{ga_end_date}&ids=ga:#{ga_ids}&metrics=#{ga_metrics}&dimensions=#{ga_dimension}&filters=#{ga_filters2}").read)    
    tmp_data = JSON.parse(tmp_data.to_json)["rows"]
    tmp_data.each do |d|
      custom_regex = "#{d[0]}<__>#{d[1]}"
      if d[2].to_i > 0
    ...
    ga_max_result = 25
    ga_start_year = Date.parse(ga_start_date).year
    quarter_hash  = {"q1" => ["01-01", "03-31"], "q2" => ["04-01", "06-30"], "q3" => ["07-01","09-30"], "q4" => ["10-01", "12-31"]}
    for l_year in ga_start_year..Date.today.year
      to_quarter = 4
      to_quarter = ((((Date.today.at_beginning_of_month - 1).month - 1) / 3) + 1) if l_year == Date.today.year
    ...
    data.each do |data_element|
      ...
      if skip_value[skip_val] < 10
        ...
        if g["success"]
          ...
          obj_key = "#{quarter}<__>#{year}<__>#{title}"
          if !ten_records_arr[obj_key]
            ten_records_arr[obj_key] = {"title" => title, "img_url_path" => img_url_path, "views" => views, "page_path" => p_path, "quarter" => quarter, "year" =>  year, "counter" => 1}
