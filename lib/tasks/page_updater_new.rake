  ...
  task :ga_traffic, [:name, :id, :provider_type,:wiki_name,:start_date,:end_date]  do |t, args|
    ...
    #     old_page_view_data_arr2 =  JSON.parse(data_filz.content)
    #     page_view_data_arr2.each {|k| old_page_view_data_arr2 << k}
    #     data_filz.update_attributes({content: old_page_view_data_arr2})
    #   end
    # end
    # #adding to viz
    ...
    #     end
    #     Data::Filz.find(data_filz.id).update_attributes({content: final_media_type_data_formatted.to_json})
    #   end
    ...
    # else
    #   #page_country_data_arr = JSON.parse(page_country_data_arr)
    #   old_page_country_data_arr = JSON.parse(data_filz.content)
    #   page_country_data_arr.shift
    #   page_country_data_arr.each{|k| old_page_country_data_arr << k}
    #   Data::Filz.find(data_filz.id).update_attributes({content: old_page_country_data_arr})
    # end
    ...
    if data_filz.nil?
      ...
      old_top_ten.each do |old|
        if !tmp["#{old[0]}+#{old[1]}+#{old[3]}+#{old[4]}+#{old[5]}"].present?
          tmp["#{old[0]}+#{old[1]}+#{old[3]}+#{old[4]}+#{old[5]}"] = old[2].to_i
        else
          tmp["#{old[0]}+#{old[1]}+#{old[3]}+#{old[4]}+#{old[5]}"] = tmp["#{old[0]}+#{old[1]}+#{old[3]}"].to_i + old[2].to_i
        end
      end

      final_top_ten_digital_objects.each do |data|
        if !tmp["#{data[0]}+#{data[1]+data[3]}+#{data[4]}+#{data[5]}"].present?
          tmp["#{data[0]}+#{data[1]+data[3]}+#{data[4]}+#{data[5]}"] = data[2].to_i
        else
          tmp["#{data[0]}+#{data[1]+data[3]}+#{data[4]}+#{data[5]}"] = tmp["#{data[0]}+#{data[1]+data[3]}+#{data[4]}+#{data[5]}"].to_i + data[2].to_i
        end
      end
      tmp.each do |key,value|
        tmp_array = []
        ...
        tmp_array[4] = key.split("+")[3]
        tmp_array[5] = key.split("+")[4]
